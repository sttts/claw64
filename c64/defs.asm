...
.const GUARD_CODE_LIMIT = $9172
.const EXEC_STAGE_BASE = $9180
.const EXEC_STAGE_LIMIT = EXEC_STAGE_BASE + $80
.const GUARD_USERQ_ENQUEUE = GUARD_CODE_BASE
.const GUARD_USERQ_LOAD = GUARD_CODE_BASE + $3B
.const GUARD_BSOUT_DRAIN = GUARD_CODE_BASE + $99
.const GUARD_CLEAR_STALE_STATUS_WAIT = GUARD_CODE_BASE + $120
.const GUARD_SET_BRF_SRC_EXEC = GUARD_CODE_BASE + $15A
.const GUARD_SET_BRF_SRC_RXBUF = GUARD_CODE_BASE + $165
...
.const USERQ_META_BASE = USERQ_LIMIT
.const USERQ_STAGE_LEN = USERQ_META_BASE + 0
.const USERQ_HEAD_PTR  = USERQ_META_BASE + 1
.const USERQ_TAIL_PTR  = USERQ_META_BASE + 2
.const USERQ_COUNT_PTR = USERQ_META_BASE + 3
.const USERQ_META_LIMIT = USERQ_META_BASE + 4
.const BUSY_COLORS_BASE = USERQ_META_LIMIT
.const BUSY_COLORS_LIMIT = BUSY_COLORS_BASE + 4
.const READY_CODES_BASE = BUSY_COLORS_LIMIT
.const READY_CODES_LIMIT = READY_CODES_BASE + 6
.const STATE_READY_TEXT_BASE = READY_CODES_LIMIT
.const STATE_READY_TEXT_LIMIT = STATE_READY_TEXT_BASE + 6
.const STATE_RUNNING_TEXT_BASE = STATE_READY_TEXT_LIMIT
.const STATE_RUNNING_TEXT_LIMIT = STATE_RUNNING_TEXT_BASE + 7
.const STATE_BUSY_TEXT_BASE = STATE_RUNNING_TEXT_LIMIT
.const STATE_BUSY_TEXT_LIMIT = STATE_BUSY_TEXT_BASE + 4
.const STATE_STORED_TEXT_BASE = STATE_BUSY_TEXT_LIMIT
.const STATE_STORED_TEXT_LIMIT = STATE_STORED_TEXT_BASE + 6
.const STATE_STOP_TEXT_BASE = STATE_STORED_TEXT_LIMIT
.const STATE_STOP_TEXT_LIMIT = STATE_STOP_TEXT_BASE + 14
